...
/**
 * Usage: AT_Weapon &lt;weaponId&gt; &lt;sellAbove&gt; &lt;buyBelow&gt;
 * Use -1 for either threshold to leave it unset. Use both -1 to remove the rule.
 */
public class AT_Weapon implements BaseCommand {
    ...
    public CommandResult runCommand(String args, CommandContext context) {
        ...
        if (spec == null) {
            Console.showMessage("Unknown weapon id: " + id);
            return CommandResult.ERROR;
        }

        ATConfig cfg = ATRegistry.get();
        if (sellAbove < 0 && buyBelow < 0) {
            cfg.weapons.remove(id);
            Console.showMessage("Removed rule for " + spec.getWeaponName() + " (" + id + ").");
        } else {
            cfg.weapons.put(id, new ATItemRule(sellAbove, buyBelow));
            Console.showMessage("Set " + spec.getWeaponName() + " (" + id + "): "
